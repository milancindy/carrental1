...
import top.naccl.common.exception.XmallException;
import top.naccl.common.utils.DtoUtil;
import top.naccl.common.utils.IDUtil;
...
public class OrderServiceImpl implements OrderService {
    ...
    public Long createOrder(OrderInfo orderInfo) {
        ...
        for(CartProduct cartProduct:list){
            ...
            //删除购物车中含该订单的商品
            try{
               // List<String> jsonList = jedisClient.getMapByHash(CART_PRE + ":" + orderInfo.getUserId());
//                List<CartProduct> jsonList = jedisClient.getListByValue(CART_PRE + ":" + orderInfo.getUserId());
                Map<String,String> mapByHash = jedisClient.getMapByHash(CART_PRE + ":" + orderInfo.getUserId());
                for (Map.Entry<String, String> entry : mapByHash.entrySet()) {
                    CartProduct cart = new Gson().fromJson(entry.getValue().toString(),CartProduct.class);
                    if(cart.getProductId().equals(cartProduct.getProductId())){
                        jedisClient.deleteByHashKey(CART_PRE + ":" + orderInfo.getUserId(),cart.getProductId()+"");
                    }
                }
            }catch (Exception e){
                e.printStackTrace();
